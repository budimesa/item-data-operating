<template>
          ...
          <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md" @click.stop>
            ...
            <form @submit.prevent="updatePost">
              <div class="mb-4">
                ...
                <input type="text" id="brand_name" v-model="currentPost.brand_name" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
              </div>
              <div class="mb-4">
                <label for="brand_code" class="block text-sm font-medium text-gray-700">Brand Code</label>
                <textarea id="brand_code" v-model="currentPost.brand_code" rows="4" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"></textarea>
              </div>
  ...
  const currentPost = ref({ brand_name: '', brand_code: '' });
  ...
  const updatePost = async () => {
    const url = route('brands.update', currentPost.value.id);
    try {
      const response = await axios.put(url, {
        brand_name: currentPost.value.brand_name,
        brand_code: currentPost.value.brand_code,
      });
