          ...
          <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md" @click.stop>
            ...
            <form @submit.prevent="update">
              <div class="mb-4">
                ...
                <input type="text" id="color_name" v-model="currentSize.color_name" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
              </div>
              <div class="mb-4">
                <label for="color_code" class="block text-sm font-medium text-gray-700">Color Code</label>
                <textarea id="color_code" v-model="currentSize.color_code" rows="4" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"></textarea>
              </div>
          ...
          <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md" @click.stop>
            ...
            <form @submit.prevent="create">
              <div class="mb-4">
                ...
                <input type="text" id="new_color_name" v-model="newSize.color_name" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
              </div>
              <div class="mb-4">
                <label for="new_color_code" class="block text-sm font-medium text-gray-700">Color Code</label>
                <textarea id="new_color_code" v-model="newSize.color_code" rows="4" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"></textarea>
              </div>
              <div class="flex justify-end space-x-2">
...
const currentSize = ref({ color_name: '', color_code: '' });
const newSize = ref({ color_name: '', color_code: '' });
...
const openCreateModal = () => {
  newSize.value = { color_name: '', color_code: '' };
...
const update = async () => {
  const url = route('colors.update', currentSize.value.id);
  try {
    const response = await axios.put(url, {
      color_name: currentSize.value.color_name,
      color_code: currentSize.value.color_code,
    });
...
const create = async () => {
  try {
    const response = await axios.post(route('colors.store'), {
      color_name: newSize.value.color_name,
      color_code: newSize.value.color_code,
    });
    await fetchData()
    closeCreateModal();
